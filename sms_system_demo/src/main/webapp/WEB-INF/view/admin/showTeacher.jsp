				...
				<div class="panel panel-default">
				    <div class="panel-heading">
						<div class="row">
							...
							<form class="bs-example bs-example-form col-md-5" role="form" action="admin/showTeacher" style="margin: 20px 0 10px 0;" id="form1" method="post">
								<div class="input-group">
									<input type="text" class="form-control" placeholder="请输入教师姓名" name="username">
				    ...
				    <table class="table table-bordered">
							...
							<c:forEach  items="${teacherList}" var="item">
								<tr>
									...
									<td>${item.username}</td>
									<td>${item.sex}</td>
									<td><fmt:formatDate value="${item.birthyear}" pattern="yyyy-MM-dd" dateStyle="medium" /></td>
									<td>${item.degree}</td>
									<td>${item.title}</td>
									<td><fmt:formatDate value="${item.grade}" pattern="yyyy-MM-dd" dateStyle="medium" /></td>
									<td>${item.collegeName}</td>
									<td>
										<button class="btn btn-default btn-xs btn-info" onClick="window.location.href='admin/editTeacher?id=${item.userid}'">修改</button>
										<button class="btn btn-default btn-xs btn-danger btn-primary" onClick="window.location.href='admin/removeTeacher?id=${item.userid}'">删除</button>
										<!--弹出框-->
									</td>
								</tr>
							</c:forEach>
